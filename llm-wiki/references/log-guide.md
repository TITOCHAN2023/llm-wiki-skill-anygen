...
- Retargeted `Anygen Code Architecture` → `tech/anygen-code/Anygen_Code_Architecture.md` in 2 files
```
...
- Body is a short bullet list summarising what changed. Reference files touched with standard MD links (`[name](../wiki/concepts/Name.md)` — path relative to `log/YYYYMMDD.md`, so two up into wiki-root then down).

## Ops allowed in the log

| Op | When it appears | Example |
|---|---|---|
| `compile`  | Structural edits, splits, merges, index rebuild | `## [10:00] compile \| split Anygen Code page into 7 sub-pages` |
...
| `split`    | A single page split into a folder | `## [10:00] split \| Anygen Code → anygen-code/` |
...
- **Long rationale**: put design decisions and rationale in `ANYGEN.md` "Notes for the LLM", not in the log.
